...
<?php
  ...
  <?php  // Vérifiez si l'utilisateur est connecté
  $username = null;
  if (isset($_SESSION['utilisateur_ID'])) {
    $userId = $_SESSION['utilisateur_ID'];    // Récupération des informations de l'utilisateur
    $sqlUser = "SELECT * FROM utilisateur WHERE utilisateur_ID=:utilisateur_ID";
    $stmtUser = $pdo->prepare($sqlUser);
    $stmtUser->execute(['utilisateur_ID' => $userId]);
    $user = $stmtUser->fetch(PDO::FETCH_ASSOC);    if ($user) {
  ...
  ">
    ...
    <nav class="navOne">
      ...
      <div class="navTwo">
        ...
        </button>        <?php if ($username): ?>
        <?php else: ?>
          <a href="connexion.php" class="position">Connexion</a>
        <?php endif; ?>        <!-- Bouton Panier et gestion de celui ci -->        <button class="position1" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasScrolling" aria-controls="offcanvasScrolling">
          Mon Panier <?php echo isset($_SESSION['panier']) ? '(' . count($_SESSION['panier']) . ')' : '(0)'; ?>
        </button>        <div class="offcanvas offcanvas-start" data-bs-scroll="true" data-bs-backdrop="false" tabindex="-1" id="offcanvasScrolling" aria-labelledby="offcanvasScrollingLabel">
          ...
          <div class="offcanvas-body">
            ...
            <?php else : ?>
              <div class="panier-items">
                ...
                <?php endforeach; ?>                <div class="total-section">
                  <h6>Total : <?php echo number_format($total, 2); ?>€</h6>
                </div>                <div class="panier-actions">
